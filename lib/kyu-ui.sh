#!/usr/bin/env bash
# Capa de interfaz (gum) del instalador de Kyu OS.
#   Se SOURCEA desde setup_master.sh y desde preview/kyu-ui-preview.sh,
#   para que ambos usen exactamente la misma UI.
#   Expone kyu_wizard: idioma → modo → paquetes → tema → resumen → confirmar.
#   Setea variables globales (KYU_LANG, KYU_MODE, KYU_PKGS, KYU_THEME) y
#   devuelve 0 (confirmado) o 1 (cancelado).
#   Lee KYU_DGPU (1 si hay GPU dedicada) para ofrecer "Rendimiento gráfico"
#   y KYU_SDBOOT (0 si NO hay systemd-boot) para avisar del branding.
#   Si KYU_UI_PREVIEW=1: no instala gum solo, y el cierre dice "preview".
...
_kui_load_strings() {
  if [[ "$KYU_LANG" == "es" ]]; then
    KS[header]="Kyu OS · Instalador"
    KS[s1]="1 · Modo de instalación"; KS[modeq]="¿Cómo quieres instalar Kyu OS?"
    ...
    KS[perf]="Rendimiento gráfico — supergfxctl · nvtop · kyu-power"
    KS[s3]="3 · Tema"
    ...
    KS[header]="Kyu OS · Installer"
    KS[s1]="1 · Install mode"; KS[modeq]="How do you want to install Kyu OS?"
    ...
    KS[perf]="Graphics performance — supergfxctl · nvtop · kyu-power"
    KS[s3]="3 · Theme"
...
# Opciones de tema (display por idioma) y su clave canónica para kyu-tema
_kui_theme_options() {
  if [[ "$KYU_LANG" == "es" ]]; then
...
_kui_theme_key() {  # display elegido → clave canónica (la que entiende kyu-tema), "__custom__" o "" (sin tema)
  case "$1" in
...
kyu_wizard() {
  ...
  KYU_LANG=""; KYU_LANG_NAME=""; KYU_MODE=""; KYU_THEME=""; KYU_PKGS=()
  : "${KYU_DGPU:=0}"; : "${KYU_SDBOOT:=1}"

  # Paso 0 — idioma PRIMERO (pantalla neutra)
  clear
  _kui_title "Kyu OS · Installer / Instalador"
  local _pick
  _pick=$(_kui_choose --header "Language / Idioma:" "English" "Español") || return 1
  if [[ "$_pick" == "Español" ]]; then KYU_LANG="es"; KYU_LANG_NAME="Español"; else KYU_LANG="en"; KYU_LANG_NAME="English"; fi
  _kui_load_strings

  # A partir de aquí todo en el idioma elegido
  clear
  _kui_title "${KS[header]}"
  [ "$KYU_SDBOOT" = "0" ] && _kui_note "⚠ ${KS[sdbootw]}"
  echo
  ...
  case "$_modo" in
    "${KS[full]}")   KYU_MODE="full"   ;;
    "${KS[custom]}") KYU_MODE="custom" ;;
    "${KS[min]}")    KYU_MODE="min"    ;;
  esac
  echo

  # Paso 2 — paquetes (solo personalizada)
  if [[ "$KYU_MODE" == "custom" ]]; then
    _kui_step "${KS[s2]}"
    _kui_note "${KS[core]}"
    _kui_hint "${KS[pick]}"
    local _opc=("${KS[cos]}" "${KS[apps]}" "${KS[gaming]}")
    [ "$KYU_DGPU" = "1" ] && _opc+=("${KS[perf]}")
    local _sel=(); mapfile -t _sel < <(_kui_choose --no-limit --header "${KS[pkgq]}" --selected "${KS[cos]}" "${_opc[@]}")
    local x
    for x in "${_sel[@]}"; do
      case "$x" in
        "${KS[cos]}")    KYU_PKGS+=("cosmeticos") ;;
        "${KS[apps]}")   KYU_PKGS+=("apps") ;;
        "${KS[gaming]}") KYU_PKGS+=("gaming") ;;
        "${KS[perf]}")   KYU_PKGS+=("rendimiento") ;;
      esac
    done
    echo
  fi
  ...
  KYU_THEME="$(_kui_theme_key "$_tpick")"
  echo

  # Resumen
  local _modname="$KYU_MODE"; case "$KYU_MODE" in
    full)   _modname="${KS[full]%% —*}"   ;;
    custom) _modname="${KS[custom]%% —*}" ;;
    min)    _modname="${KS[min]%% —*}"    ;;
  esac
  local RES="${KS[rmode]} ${_modname}
${KS[rlang]} ${KYU_LANG_NAME}
${KS[rtheme]} ${_tpick}"
  if [[ "$KYU_MODE" == "custom" ]]; then
    if [ ${#KYU_PKGS[@]} -gt 0 ]; then
      RES+=$'\n'"${KS[rpkgs]} $(printf '%s, ' "${KYU_PKGS[@]}" | sed 's/, $//')"
    else
      RES+=$'\n'"${KS[rpkgs]} ${KS[rcore]}"
    fi
  fi
